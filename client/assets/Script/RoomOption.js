...
cc.Class({
    ...
    init: function (type, mode, refresh_fangka_cb) {
    ...
    refresh: function () {
        ...
        for (let k in this.option) {
            let toggle_container = this.option[k]
            toggle_container.layout.removeAllChildren()
            toggle_container.node = []
            let type = toggle_container.type
            ...
            if (toggle_container.options.length) {
                ...
                let option_num = toggle_container.options.length
                var  ischose=false;
                for (let i = 0; i < option_num; i++) {
                    let toggle = cc.instantiate(this.toggle_prefab)
                    let bg_sprite = type == "unique" ? this.unique_bg_sprite : this.multi_bg_sprite
                    let checkmark_sprite = type == "unique" ? this.unique_checkmark_sprite : this.multi_checkmark_sprite
                    let click_cb = type == "unique" ? this.unique_toggle_click : this.multi_toggle_click
    ...
    get_fangka_pay_num() {
        if (this.mode == "sss") {
            if (this.option_dict.pay == 4) return 0
            else if (this.option_dict.pay == 1 || this.option_dict.pay == 3) {
            ...
            if (this.option_dict.pay == 1) return 0
            else if (this.option_dict.pay == 2) {
    ...
    unique_toggle_click(container, event_node) {
        for (let i = 0; i < container.node.length; i++) {
    ...
    multi_toggle_click(container, event_node) {
        let cs = event_node.getComponent("Toggle")
    ...
    default_option: function (type, mode) {
        this.key = type + ":" + mode
        let option_dict = cc.sys.localStorage.getItem(this.key)
        if (!option_dict) {
            if (mode == "ndmj") {
                option_dict = {
                    round: 8,
    ...
    option_conf: function (type, mode) {
        var OPTION = {
            fzmj: {
                INTEGRAL_CLUB: {
