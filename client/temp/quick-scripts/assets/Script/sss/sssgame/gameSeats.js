(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/sss/sssgame/gameSeats.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '5119cs9zp9KYpDKAUEpw3+J', 'gameSeats', __filename);
// Script/sss/sssgame/gameSeats.js
...
cc.Class({
    ...
    deleteOneSeat: function deleteOneSeat(data) {
        cc.log(data);
        var seatData = cc.sssNetMgr.seats;
        for (var i = 0; i < seatData.length; i++) {
    ...
    setResultData: function setResultData(data) {
        ...
        //分数显示
        var setAllScore = function setAllScore(target, data) {
        ...
        //显示本局结算
        var showOver = function showOver(target, data) {
    ...
    showTSPResult: function showTSPResult(arrSeq, TSPResult) {
        if (!TSPResult || TSPResult.length < 1) {
            return;
        }
        for (var i = 0; i < TSPResult.length; i++) {
            ...
            var showTSP = function showTSP(target, data) {
    ...
    showShootResult: function showShootResult(arrSeq, shootResult) {
        cc.log(shootResult);
        ...
        for (var key in shootInfo) {
            ...
            for (var i = 0; i < beShootArr.length; i++) {
                ...
                data.index0 = index0;
                data.index1 = index1;
                var setShoot = function setShoot(target, data) {
        ...
        var setAllScore = function setAllScore(target, data) {
            for (var _i = 0; _i < data.length; _i++) {
                var userId = data[_i].userId;
                var score = data[_i].score;
                var seatIndex = cc.sssNetMgr.getSeatIndexByID(userId);
                var index = cc.sssNetMgr.getLocalIndex(seatIndex);
                if (!target.getComponent('gameSeats').seatComs[index]) {
                    return;
        ...
        if (shootResult.swatUserId > 0) {
            ...
            var showSwat = function showSwat(target, data) {
    ...
    showCompareResult: function showCompareResult(arrSeq, tdRes, zdRes, wdRes) {
        if (tdRes.length < 1 || zdRes.length < 1 || wdRes.length < 1) {
            return;
        }
        for (var i = 0; i < tdRes.length; i++) {
            var data = {};
            arrSeq.push(cc.delayTime(0.5));
            if (tdRes[i]) {
            ...
            var setTD = function setTD(target, data) {
        ...
        for (var _i2 = 0; _i2 < zdRes.length; _i2++) {
            var data = {};
            arrSeq.push(cc.delayTime(0.5));
            if (zdRes[_i2]) {
                var userID = zdRes[_i2].userId;
                var score = zdRes[_i2].score;
                var arrPai = zdRes[_i2].arrPai;
                var type = this.getPaiType(arrPai);
            ...
            var setZD = function setZD(target, data) {
                if (!target.getComponent('gameSeats').seatComs[data.comIndex]) {
                    return;
                }
        ...
        for (var _i3 = 0; _i3 < wdRes.length; _i3++) {
            var data = {};
            arrSeq.push(cc.delayTime(0.5));
            if (wdRes[_i3]) {
                var userID = wdRes[_i3].userId;
                var score = wdRes[_i3].score;
                var arrPai = wdRes[_i3].arrPai;
                var type = this.getPaiType(arrPai);
            ...
            var setWD = function setWD(target, data) {
                if (!target.getComponent('gameSeats').seatComs[data.comIndex]) {
                    return;
                }
    ...
    //对牌型牌值进行排序
    arrSort: function arrSort(arrRes, arrTar) {
    ...
    getRealType: function getRealType(paiType, dunIndex) {
        switch (dunIndex) {
            case 0:
                //头道
    ...
    //获取牌的类型
    getPaiType: function getPaiType(arrPai) {
        var arrType = [];
    ...
    //以牌值和牌型对比大小
    comparePai: function comparePai(arrPai1, arrPai2) {
    ...
    setScore: function setScore(data) {
        //特殊牌显示完之后的分数显示
...
cc._RF.pop();
        ...
        //# sourceMappingURL=gameSeats.js.map
