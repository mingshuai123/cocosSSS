(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/Script/sss/sssgame/Compare.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '273c1I6VVFPg63gmysKDdIF', 'Compare', __filename);
// Script/sss/sssgame/Compare.js
    ...
    if (!arrTemp0 && arrResultPai.length > 1) {
        for (var x = 1; x < arrResultPai.length; x++) {
            ...
            for (var y = 0; y < arrResultPai[0].length; y++) {
                if (Number(arrResultPai[0][y].value) > Number(arrResultPai[x][y].value)) {
                    break;
...
Compare.getType = function (arrPai) {
    ...
    var samePaiCnt = function samePaiCnt() {
        var len = arrPai.length;
        if (len !== 13) {
            return 0;
        }
        ...
        for (var i = 0, l = len; i < l; i++) {
            var item = arrPai[i].value;
            result[item] = result[item] + 1 || 1;
        }
        var sameCnt = 0;
        for (var i in result) {
            var _sameCnt = result[i];
            if (_sameCnt > sameCnt) {
                sameCnt = _sameCnt;
            }
        }
    ...
    var wanfatype = cc.sssNetMgr.conf.type;
    cc.log(wanfatype);
    var sameCnt = samePaiCnt();
    if (sameCnt >= 8) {
        type = PaiType.BXGH;
        arrResultType.push(PaiType.BXGH);
    } else if (sameCnt == 7) {
        type = PaiType.QXGZ;
        arrResultType.push(PaiType.QXGZ);
    } else if (sameCnt == 6) {
        type = PaiType.LLDS;
        arrResultType.push(PaiType.LLDS);
    }
...
Compare.compareSameType = function (arrPai1, arrPai2, type) {
    ...
    //         if (Number(arrPai1[i].value) > Number(arrPai2[i].value)){
    //             return 1;
    //         }
    //         else if (Number(arrPai1[i].value) < Number(arrPai2[i].value)){
    //             return -1;
    //         }
    //     }
    //     return 0;
